...
export const AssessmentEditorScreen = () => {
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      updatePackMeta({ assessmentPack: pack } as Partial<typeof currentPack.meta>);
      setIsDirty(false);
                  ...
                  <div className="px-4 pb-4 space-y-3 border-t border-zinc-100 pt-3">
                    ...
                    <div>
                      ...
                      <Textarea
                        value={q.answer ?? ""}
                        onChange={(e) =>
                          updateQuestion(idx, "answer", e.target.value || null)
                        }
                        rows={2}
                        className="resize-y"
                      />
                ...
                {pack.homework.tasks.map((task, idx) => (
                  ...
                  >
                    <CollapsibleTrigger className="w-full flex items-center gap-3 p-4 text-left hover:bg-zinc-50 transition-colors">
                      ...
                      <span className="text-sm text-zinc-800 flex-1 truncate">
                        {task.task_text.slice(0, 100)}{task.task_text.length > 100 ? "…" : ""}
                      </span>
                      <span className="text-xs text-zinc-400 shrink-0">
                      ...
                      <div className="px-4 pb-4 space-y-3 border-t border-zinc-100 pt-3">
                        ...
                        <div>
                          ...
                          <Textarea
                            ...
                            onChange={(e) =>
                              updateHomeworkTask(idx, "task_text", e.target.value)
                            }
                            rows={2}
                            className="resize-y"
                          />

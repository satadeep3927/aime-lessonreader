...
import { CheckCircle2, HelpCircle, Lightbulb, PenTool, Target } from "lucide-react";
...
export const GenericSlide = ({ slide }: GenericSlideProps) => {
  const renderIcon = () => {
    switch (slide.slide_type) {
      case "TEACH": return <Lightbulb className="w-8 h-8 text-amber-500 dark:text-amber-400" />;
      case "HOOK": return <HelpCircle className="w-8 h-8 text-purple-600 dark:text-purple-400" />;
      case "LEARNING_OBJECTIVES": return <Target className="w-8 h-8 text-green-600 dark:text-green-400" />;
      case "WORKED_EXAMPLE": return <PenTool className="w-8 h-8 text-blue-600 dark:text-blue-400" />;
      default: return <CheckCircle2 className="w-8 h-8 text-gray-500 dark:text-gray-400" />;
    }
  };
                ...
                {'questions' in slide && (
                    <div className="space-y-6">
                         {(slide as any).questions.map((q: string, i: number) => (
                             <div key={i} className="bg-white dark:bg-zinc-800 p-6 rounded-xl border border-zinc-200 dark:border-zinc-700 shadow-xs">
                                 <h4 className="text-sm font-bold uppercase tracking-widest text-zinc-400 mb-3">Question {i+1}</h4>

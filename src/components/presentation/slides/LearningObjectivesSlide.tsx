...
export const LearningObjectivesSlide = ({
  slide,
}: LearningObjectivesSlideProps) => {
  return (
    <SlideContainer>
        ...
        <div className="flex-1 min-h-0 flex flex-col justify-center max-w-4xl mx-auto w-full">
          <div className="space-y-6">
            {slide.objectives.map((obj, i) => (
              ...
              >
                <span className="shrink-0 w-10 h-10 rounded-full bg-green-100 dark:bg-green-900/40 flex items-center justify-center text-green-700 dark:text-green-400 font-bold text-lg border border-green-200 dark:border-green-800">
                  {i + 1}
                </span>
                <div className="pt-1 flex-1">
                  <MarkdownRenderer
                    content={obj}
                    className="text-2xl font-medium text-gray-800 dark:text-zinc-100 leading-snug"
                  />
                </div>
              </div>
            ))}
